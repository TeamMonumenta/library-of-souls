    ...
    doLast {
        ssh.runSessions {
            session(adminssh) {
                ...
                execute("cd /home/epic/project_epic/mobs/plugins && rm -f LibraryOfSouls.jar && ln -s ../../server_config/plugins/LibraryOfSouls.jar")
            }
        }
    }
}
    ...
    doLast {
        ssh.runSessions {
            session(adminssh) {
                ...
                execute("cd /home/epic/play/m14/server_config/plugins && rm -f LibraryOfSouls.jar && ln -s " + shadowJar.archiveFileName.get() + " LibraryOfSouls.jar")
